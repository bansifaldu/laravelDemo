...
@endif
    <div class="card-header">
        <h3 class="card-title">User Management Update</h3>
    </div>
    <!-- /.card-header -->
    <!-- form start -->
    <form id="user_manage_edit" class="form-horizontal" method="POST" action="{{ route('admin.usermagagement_edit_save') }}">
        @csrf
                ...
                <div class="col-sm-10">
                    <select class="selectpicker1 form-control col-sm-12" id="role_id" multiple data-live-search="true" name="role_id[]">
                        <?php foreach($role as $key=>$val){ ?>
                            <option value="{{ $val->id}}" <?php echo in_array($val->id,$get_role)?"selected" :"" ;?>>{{ $val->name}}</option>
                        <?php } ?>
                    </select>
                </div>
            </div>
